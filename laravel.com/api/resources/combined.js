...
$(function() {
	...
	$search
		.autocomplete(ApiGen.elements, {
			...
			var location = window.location.href.split('http://laravel.com/');
			location.pop();
			...
			location.push(file);
			window.location = location.join('http://laravel.com/');

			// Workaround for Opera bug
			$(this).closest('form').attr('action', location.join('http://laravel.com/'));
		}).closest('form')
			.submit(function() {
